...
## Set a device role
On the Pi, to set the device role to _CLIENT_ (for text).

    $ meshtastic --set device.role CLIENT

Or, to set meshtastic device role to _SENSOR_ (for data).
...
## Set a known PSK
On a second or third Pi, to set a Base64 encoded PSK, e.g. _TVlfUEFTU1dPUkQ=_ on channel _0_.
...
Wait for a sender to [send sensor data](#send-sensor-data).
